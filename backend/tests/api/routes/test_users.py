...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = crud.get_user_by_email(session=db_session, email=username)
        assert user
...
def test_get_existing_user(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = crud.get_user_by_email(session=db_session, email=username)
    assert existing_user
    assert existing_user.email == api_user["email"]


def test_get_existing_user_current_user(client: TestClient, db_session: Session) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=db_session, user_create=user_in)
...
def test_update_user_me(
    ...
    user_db = db_session.exec(user_query).first()
    assert user_db
...
def test_register_user_already_exists_error(client: TestClient, test_superuser: User) -> None:
    password = random_lower_string()
    full_name = random_lower_string()
    data = {
...
def test_delete_user_me(client: TestClient, db_session: Session) -> None:
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user = crud.create_user(session=db_session, user_create=user_in)
    ...
    user_db = db_session.execute(user_query).first()
...
def test_delete_user_current_super_user_error(
    ...
    super_user = crud.get_user_by_email(session=db_session, email=settings.FIRST_SUPERUSER)
    assert super_user
