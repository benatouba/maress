...
describe('useGraphComposable', () => {
    ...
    describe('exportToImage', () => {
        it('should return empty string if cy is null', () => {
            const result = composable.exportToImage(null);
            expect(result).toBe('');
        });
        ...
        it('should use white background for jpg format', () => {
            ...
            expect(mockPng).toHaveBeenCalledWith(expect.objectContaining({
                bg: '#FFFFFF'
            }));
        });
    ...
    describe('formatNodeLabel', () => {
        ...
        it('should return "Untitled" for null label', () => {
            const label = composable.formatNodeLabel(null, 30);
            expect(label).toBe('Untitled');
        });
    ...
    describe('calculateGraphStats', () => {
        it('should return zero stats for null cy', () => {
            const stats = composable.calculateGraphStats(null);
        ...
        it('should calculate correct stats for graph', () => {
            ...
            const mockEdges = [{}, {}];
            const mockCy = {
                nodes: () => ({ length: 3, forEach: (fn) => mockNodes.forEach(fn) }),
                edges: () => ({ length: 2 }),
                elements: () => ({
                    components: () => ({ length: 1 })
                })
        ...
        it('should round avgDegree and density', () => {
            ...
            const mockCy = {
                nodes: () => ({ length: 2, forEach: (fn) => mockNodes.forEach(fn) }),
                edges: () => ({ length: 1 }),
                elements: () => ({
                    components: () => ({ length: 1 })
                })
