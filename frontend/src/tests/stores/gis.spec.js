...
describe('GIS Store', () => {
    ...
    it('runs buffer and stores features', async () => {
        ...
        const result = await store.runBuffer({ layer_id: 'study-sites', selection: { type: 'all' } }, { distance: 1000, unit: 'meter', dissolve: false });
    ...
    it('runs clip and stores clipped study sites', async () => {
        ...
        const result = await store.runClip({ layer_id: 'study-sites', selection: { type: 'all' } }, { layer_id: 'regions', selection: { type: 'ids', ids: ['region-1'] } });
        expect(result?.count).toBe(1);
    ...
    it('runs within-distance and stores returned features', async () => {
        ...
        const result = await store.runWithinDistance({ layer_id: 'study-sites', selection: { type: 'all' } }, { layer_id: 'regions', selection: { type: 'all' } }, { distance: 1000, unit: 'meter', return: 'source' });
