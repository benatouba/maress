...
describe('Map Page', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(studySitesStore, 'fetchItemStudySites').mockImplementation(async (itemId) => {
            if (itemId === mockPaper1.id)
                return [mockStudySite];
            if (itemId === mockPaper2.id)
                return [mockStudySiteAuto];
            return [];
        });
        // Mock Zotero store
    ...
    describe('Paper Filtering', () => {
        ...
        it('should filter papers with study sites', async () => {
            ...
            expect(wrapper.vm.filteredPapers.every((p) => p.study_site_count > 0)).toBe(true);
        });
    ...
    describe('Paper Selection', () => {
        ...
        it('should filter study sites by selected paper', async () => {
            ...
            expect(filtered.every((s) => s.item_id === mockPaper1.id)).toBe(true);
        });
    });
    ...
    describe('Map Integration', () => {
        ...
        it('should fit map to markers when paper selection changes', async () => {
            ...
            const mapComponent = wrapper.findComponent({ name: 'StudySiteMap' });
            mapComponent.vm.fitToMarkers = fitToMarkersSpy;
            wrapper.vm.selectedPaper = mockPaper1;
            await wrapper.vm.$nextTick();
